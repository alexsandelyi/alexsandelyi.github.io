# 커뮤니티 게시판 — 설계

**상태: 설계 확정, 구현 전** (2026-08-13).

**로그인이 없다.** 누구나 와서 쓰고 누구나 읽는다. 한 페이지에 10개.
계정도 세션도 만들지 않는다.

## 왜 별도 페이지인가

런처 번들(`index.html`) 안의 커뮤니티 섹션에는 「글 쓰기」 버튼이 있지만
아무 동작도 없고 글 목록도 더미다. **거기에 기능을 넣으면 안 된다** —
번들은 외부 도구가 재생성하므로 손으로 넣은 코드는 다음 재생성 때 사라진다
(루트 `CLAUDE.md`). `games/soccer/` 와 같이 번들 밖 독립 페이지로 만들고,
런처 커뮤니티 섹션의 **하드코딩된 가짜 글 3개는 숨긴다**
(`patch-hero.js`). 실제 게시판과 아무 관계가 없고 눌러도 아무 데도 가지
않는다. 진짜 글을 거기 띄우려면 번들이 API 를 호출해야 하는데 그 코드는
재생성 때 사라진다 — 배포 후에 다시 판단한다.

런처의 버튼 연결만 `tmp/patch-community.js` 로 처리한다 — 「글 쓰기」를
`<a href="community/">` 로 바꾼다. `<button>`+onClick 이 아니라 링크라야
새 탭 열기·가운데 클릭·주소 복사가 동작한다.

## 구조

```
community/            정적 프런트 — GitHub Pages 가 서빙
  index.html          독립 페이지
  board.css           목록·글쓰기 창 스타일 (두 곳 공용)
  js/01-api.js        서버 통신. **저장소를 바꾸려면 이 파일만 갈아끼운다**
  js/02-board.js      게시판 부품 — 목록·페이지네이션·작성 창
  js/embed.js         런처 안에 붙이는 얇은 어댑터

worker/               Cloudflare Worker — Pages 가 서빙하지 않는다
  src/index.js        라우팅·검증
  schema.sql          D1 테이블
  wrangler.toml
```

`js/` 를 번호로 나눠 클래식 `<script src>` 로 부른다 (축구 게임과 같다).
빌드가 필요 없다.

`01-api.js` 의 `API_BASE` 가 비어 있으면 **localStorage 가짜 저장소**로
돈다. Worker 없이 화면을 다 만들고 검증하려는 것이고, 응답 모양이 진짜와
같아서 나중에 주소 한 줄만 넣으면 갈린다.

가짜 저장소도 **비밀번호를 해시로** 들고 확인한다. 평문으로 두면
localStorage 를 열어보는 것만으로 지워지고, 틀린 비밀번호 경로를 시험할
수도 없다. (소금·후추를 곁들인 진짜 확인은 Worker 가 한다 — [api.md](api.md))

## 게시판은 런처 안에서 돈다

「글 쓰기」를 누르면 **이동하지 않고 그 자리에서 창이 뜨고**, 올리면 런처의
커뮤니티 섹션에 바로 쌓인다. 독립 페이지(`community/`)도 그대로 남아 있다.

두 곳이 **같은 부품**(`02-board.js`)을 쓴다 — 두 벌로 만들면 한쪽만 고치는
일이 생긴다. `createBoard()` 에 그릴 자리를 넘기면 어디든 붙는다.

**번들에는 `<script src>` 네 줄만 넣는다.** 런처는 외부 도구가 재생성하므로
안에 코드를 넣으면 사라진다. 로직은 전부 번들 밖 `community/` 에 두고,
재생성되면 그 네 줄만 다시 넣는다 (`tmp/patch-community.js`).

주의할 것 둘.

- **React 가 섹션을 다시 그리면 우리 것이 지워진다.** `embed.js` 가
  `MutationObserver` 로 보고 있다가 다시 붙인다.
- 런처에서는 주소에 `?page=` 를 **쓰지 않는다**. 게시판이 런처의 주소를
  바꾸면 안 된다. 독립 페이지에서만 켠다 (`syncUrl`).

## 호스팅 — 서브도메인을 쓴다

| 이름 | 가리키는 곳 | Cloudflare 프록시 |
|---|---|---|
| `ilbbang.com` | GitHub Pages | **끔** (지금 그대로) |
| `www.ilbbang.com` | GitHub Pages | **끔** |
| `api.ilbbang.com` | Worker | **켬** (Worker 는 프록시가 필요) |

**기존 설정을 건드리지 않는 것이 요점이다.** `ilbbang.com/api/*` 로 Worker
경로를 잡으려면 apex 에 프록시를 켜야 하는데, 그러면 GitHub Pages 의 인증서
갱신(90일마다 HTTP 검증)에 위험이 생긴다. 지금 잘 도는 것을 굳이 흔들지
않는다.

대신 출처가 달라지므로 **CORS** 가 필요하다. Worker 가
`Access-Control-Allow-Origin: https://ilbbang.com` 을 붙인다.

## 서버 설계는 따로

데이터 모델, 글 비밀번호, API 목록, 악용 대책, Worker 검증은
[api.md](api.md) 에 있다.

## 프런트 동작

- 주소에 `?page=2` 를 반영한다 — 새로고침·뒤로가기·링크 공유가 된다
  (독립 페이지에서만. 런처에서는 끈다)
- 한 페이지 **10개 고정**
- **로딩 / 빈 목록 / 오류**를 각각 그린다. 서버가 죽었을 때 빈 화면만
  나오면 원인을 알 수 없다
- 작성은 목록 위의 「글 쓰기」로 연다
- **글을 누르면 내용 창이 뜬다.** 목록 응답에 본문이 이미 들어 있어
  추가 요청을 하지 않는다
- 카드는 `<div>` 가 아니라 **`<button>`** 이다. 클릭만 달면 키보드·스크린
  리더로는 누를 수 없다
- 본문 줄바꿈은 `white-space: pre-wrap` 으로 살린다. `<br>` 로 바꿔
  `innerHTML` 에 넣으면 그 순간 XSS 가 열린다
- **삭제는 내용 창에서 한다.** 로그인이 없으므로 글을 쓸 때 정한 비밀번호가
  본인이 자기 글을 지우는 유일한 수단이다
  - 비밀번호 칸은 **접어 둔다**. 늘 펴 두면 잘못 누르기 쉽고, 읽으러 온
    사람에게 비밀번호 칸부터 보이는 것도 이상하다
  - 「삭제」는 「닫기」와 **떨어뜨려** 놓는다 (`margin-right:auto`)
  - 틀리면 **창을 닫지 않는다.** 다시 칠 수 있어야 한다
  - 전송 중에는 잠근다 — 연타해도 요청은 한 번뿐이다
  - 지운 뒤 **보던 페이지를 다시 부른다.** 마지막 글을 지워 그 페이지가
    사라지면 서버가 마지막 페이지로 당겨주고 주소도 따라 맞춘다

## 만드는 순서

1. D1 만들고 스키마 넣기 — 로컬에서 `wrangler d1 execute --local` 로 확인
2. Worker 의 `GET /api/posts` 만 먼저. 더미 데이터로 목록·페이지네이션 완성
3. 프런트 목록 화면 (로딩·빈·오류 포함)
4. `POST /api/posts` + Turnstile + 속도 제한 + 서버 검증
5. 글 비밀번호로 수정·삭제, 신고 — **삭제는 완료** (수정·신고는 API 만 있고
   화면이 없다)
6. `api.ilbbang.com` 연결, CORS 확인
7. ~~런처 버튼 연결~~ — **완료.** `tmp/patch-community.js` 가 「글 쓰기」를
   `community/` 링크로 바꾼다

## 검증

- 글 0개 / 1개 / 10개 / 11개 / 137개에서 페이지 번호가 맞는가
- `?page=999` 처럼 없는 페이지에 들어가면 어떻게 되는가
- 제목에 `<script>` 를 넣어도 실행되지 않는가
- 같은 사람이 연속으로 쓰면 막히는가
- 틀린 비밀번호로 남의 글을 지울 수 있는가 (반드시 실패해야 한다)
- 비밀번호를 반복해서 틀리면 막히는가
- 서버가 죽었을 때 화면에 오류가 뜨는가
- 모바일 세로에서 목록·작성 폼이 쓸 만한가
