...
그림만 생성 도구로 받고, 배경 제거부터 시트 통합·검증까지는 자동이다.
아래 표는 **아직 손 일러스트로 바꾸지 못한 10포즈**의 프롬프트다. 지금
시트에는 픽셀아트 생성본이 들어가 있는데 크기가 맞지 않는다 —
[sprite-sources.md](sprite-sources.md) 「생성 포즈」 참조.
...
## 픽셀아트 생성기의 한계 (2026-08-13)

지금 시트의 10포즈는 `sprite-ai` MCP 로 뽑았다. **붙기는 붙지만 권하지
않는다.** 다음에 원본을 다시 구할 때 무엇을 피해야 하는지 적어둔다.

도구가 픽셀아트 전용이라 24색 팔레트에 1px 검정 아웃라인이 나온다
(손 일러스트 `run.png` 은 22,526색 셀셰이딩). 그런데 이 게임은:
...
따르면 프레임이 17 → **136** (8방향)이 되고 `ctx.rotate` 를 방향→칸 선택
으로 바꿔야 한다. 아트 교체가 아니라 엔진 변경이다.

**매끈한 셀셰이딩을 내는 범용 이미지 생성 도구가 낫다.** 자유 회전과 축소를
견디는 그림이어야 한다는 것이 이 파이프라인의 전제다.

프롬프트에서 배운 것 둘:

- `camera_perspective: top_down` 파라미터만으론 **눈높이 뒷모습**이 나온다.
  "카메라가 바로 위에서 내려다본다 / 정수리와 어깨가 먼저 보인다"를 문장
  으로 박아야 각도가 나온다.
- **배경은 투명이어도 된다.** 위 표의 「흰 배경」은 flood fill 을 위한
  것인데, `sprite-cutout.py` 의 `nearwhite()` 가 알파 8 미만도 배경으로
  치므로 투명 배경이 그대로 통과한다.
